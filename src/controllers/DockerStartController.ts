...
class DockerStartController {
  ...
  async handle(req: Request, res: Response): Promise<void> {
    ...
    await this.dockerStartService.execute(containerId)
      .then(() => res.status(200).json({ message: `Container ${containerId} iniciado com sucesso!`, success: true }))
      .catch((err) => res.status(400).json({ message: `Falha ao iniciar o container ${containerId} - ${err}`, success: false }))
  }
}
